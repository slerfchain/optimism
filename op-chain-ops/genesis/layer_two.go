...
func BuildL2Genesis(config *DeployConfig, l1StartBlock *types.Block) (*core.Genesis, error) {
	...
	if !db.Exist(config.GovernanceTokenOwner) {
		db.CreateAccount(config.GovernanceTokenOwner)
	}

	totalSupply := new(big.Int).Mul(new(big.Int).SetUint64(1e18), big.NewInt(499997750))

	db.AddBalance(config.GovernanceTokenOwner, totalSupply)
